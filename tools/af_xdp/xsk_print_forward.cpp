...
// - Writes a source-prefix redirect rule to 'conf' and pass default to 'settings'
...
// Pinned bpffs map locations for this standalone diagnostic helper.
static const char* PIN_CONF = "/sys/fs/bpf/xdp_fw_conf";
static const char* PIN_SETTINGS = "/sys/fs/bpf/xdp_fw_settings";
static const char* PIN_XSKS = "/sys/fs/bpf/xdp_fw_xsks";
...
static int get_pinned_map_fds(int& conf_fd, int& xsks_fd, int& settings_fd) {
    conf_fd = bpf_obj_get(PIN_CONF);
    if (conf_fd < 0) return -1;
    xsks_fd = bpf_obj_get(PIN_XSKS);
    if (xsks_fd < 0) { close(conf_fd); return -1; }
    settings_fd = bpf_obj_get(PIN_SETTINGS);
    if (settings_fd < 0) { close(conf_fd); close(xsks_fd); return -1; }
    return 0;
}
...
static bpf_object* open_bpf_object() {
    ...
    for (const char* path : candidates) {
        bpf_object* obj = bpf_object__open_file(path, nullptr);
        if (obj) return obj;
    }
    return nullptr;
}

static void set_source_prefix_rule_fd(int conf_fd,
                                      int settings_fd,
                                      uint32_t prefix,
                                      uint32_t mask,
                                      uint32_t qid) {
    ...
    BpfMatchRule rule{};
    rule.enabled = 1;
    rule.match_fields = MATCH_SRC_IP;
    rule.src_prefix = prefix;
    rule.src_mask = mask;
    rule.action = ACTION_REDIRECT_XSK;
    rule.qid = qid;
    rule.use_qid = 1;

    if (bpf_map_update_elem(conf_fd, &key, &rule, BPF_ANY)) {
        perror("bpf_map_update_elem(conf)"); exit(1);
    }

    BpfSettings settings{};
    settings.rule_count = 1;
    settings.default_action = ACTION_PASS;
    settings.default_qid = qid;
    if (bpf_map_update_elem(settings_fd, &key, &settings, BPF_ANY)) {
        perror("bpf_map_update_elem(settings)"); exit(1);
    }
}

static void bind_xsk_to_map_fd(int xsks_fd, int qid, int xsk_fd) {
    uint32_t key = static_cast<uint32_t>(qid);
    if (bpf_map_update_elem(xsks_fd, &key, &xsk_fd, BPF_ANY)) {
...
int main(int argc, char** argv) {
    ...
    int conf_fd = -1, xsks_fd = -1, settings_fd = -1;
    bpf_object*  obj  = nullptr;
    bpf_program* prog = nullptr;

    if (args.no_attach) {
        if (get_pinned_map_fds(conf_fd, xsks_fd, settings_fd) < 0) {
            fprintf(stderr, "Pinned maps not found. Attach once without --no-attach first.\n");
            return 1;
        }
        if (args.verbose) {
            fprintf(stderr, "[v] Reusing pinned maps: conf=%s settings=%s xsks=%s\n",
                    PIN_CONF, PIN_SETTINGS, PIN_XSKS);
        }
    } else {
        ...
        prog = bpf_object__find_program_by_name(obj, "xdp_redirect_openpenny");
        if (!prog) { fprintf(stderr, "program 'xdp_redirect_openpenny' not found\n"); return 1; }
        ...
        bpf_map* msettings = bpf_object__find_map_by_name(obj, "settings");
        bpf_map* mxsks = bpf_object__find_map_by_name(obj, "xsks_map");
        if (!mconf || !msettings || !mxsks) { fprintf(stderr, "maps not found in object\n"); return 1; }
        pin_map_if_needed(mconf, PIN_CONF);
        pin_map_if_needed(msettings, PIN_SETTINGS);
        pin_map_if_needed(mxsks, PIN_XSKS);
        conf_fd = bpf_map__fd(mconf);
        settings_fd = bpf_map__fd(msettings);
        xsks_fd = bpf_map__fd(mxsks);
        if (args.verbose) {
            fprintf(stderr, "[v] Pinned maps at %s, %s and %s\n",
                    PIN_CONF, PIN_SETTINGS, PIN_XSKS);
        }
    }

    set_source_prefix_rule_fd(conf_fd, settings_fd, args.prefix, args.mask, (uint32_t)args.queue);
    if (args.verbose) {
        fprintf(stderr, "[v] rule[0]: src_prefix=%u src_mask=%u action=redirect qid=%d; default=pass\n",
    ...
    bind_xsk_to_map_fd(xsks_fd, args.queue, xskfd);
    if (args.verbose) {
        fprintf(stderr, "[v] xsks_map[%d] <- fd=%d\n", args.queue, xskfd);
    }
